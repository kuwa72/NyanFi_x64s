...
**C++Builder とのクロスビルドは維持しない** (2026-08-21 にメンテナが決定。
`docs/port/decisions-needed.md` §8)。BCC64 で通るかを気にする必要はなく、
`NyanFi.cbproj` も維持しなくてよい。

それでも `src/` の変更は最小限にする。理由が変わっただけで方針は同じ:

- `src/` は 18,031行が**回帰テストで挙動を固定してある**。書き換えるほど
  「移植なのか挙動変更なのか」が曖昧になる (規約6 と同じ理由)
- `vcl_shim.h` を強制インクルードして暗黙 `vcl.h` を再現しているので、
  呼び出し側を書き換えずに済むことが多い。**まずシム側で吸収できないか考える**
- 置き換えの実例: `src/usr_shell.h` の `TItemsProperty` (`__property` → 添字プロキシ)、
  `src/usr_file_inf.cpp` の `scope_exit` (`try/__finally` → RAII)
...
`nyanfi_gui_core` (wx 非依存) に入っていないコードは**テストの対象外**で、
CI の GUI 確認は「5秒間プロセスが生きていること」しか見ない。**キーが1つも効かなくても
CI は通る。**

実際に `gui/key_map_wx.cpp` が wx の `GetKeyCode()` を仮想キーコードとして扱っており、
**英数字以外のキーが全滅していた** (報告書 §18)。実機で触るまで誰も気づかなかった。

- wx の型に触る層は**受け渡しだけ**にする。判断・変換・計算は純関数に切り出して
  `nyanfi_gui_core` へ置き、doctest で固定する
- wx の定数値を wx 非依存側に書き写すときは、`static_assert` で本物との一致を
  コンパイル時に確認する (`gui/key_map_wx.cpp`)。値を手で数えると実際にずれた
- 定数のオフセットは**コンパイラに出力させて確かめる**。ヘッダを目で数えない

### 9. 「テストが無い経路」を疑う

これまでに見つけた4件の不具合は**すべてテストが1件も無い経路**にあり、
4件とも*テストの網羅ではなく、人が触るか読むかで*見つかっている。

| 不具合 | 見つかり方 |
|---|---|
| `CopyItems` の無限再帰 (自分の配下へコピー) | 破壊的経路を読んだ |
| 画像ビューアの次/前が拡張子だけで判定 | 破壊的経路を読んだ |
| 一括リネームの2段目失敗でファイルが一時名のまま残る | 破壊的経路を読んだ |
| キー入力が英数字以外で全滅 | **メンテナが実機で触った** |

新しい層を足したら「この経路のテストは何件あるか」を先に数える。0 件なら、
テストを足すか、テストできる形に切り出すか、**報告に「未検証」と明記する**。
...
| 場所 | 内容 |
|---|---|
| `src/` | 既存の実装。回帰テストで挙動を固定してあるので変更は最小限 |
...
- **報告を受けたら成果物の実在を `ls` と `git status` で確認してからコミットする**。
  詳細な報告が来たのにファイルが存在しなかったことが実際にある
