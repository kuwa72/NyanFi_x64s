 ...
 *  - 選択状態 (`sel_list`)・階層同期 (`sync_lr`) … VCL 版は持つが、
 *    Phase 2 骨格では対応する機能自体が無い (SyncLR 未実装、TabDlg 未移植)
 *    ため保持しない。未対応として報告に明記する
 *  - カスタムキャプション・アイコン・ワークリスト指定 … `TTabSetDlg`
 *    (F:TabDlg。`gui/tab_settings.*` + `gui/tab_dialog.*`) で編集できる。
...
//---------------------------------------------------------------------------
/// 1タブ分の状態 (左右両ペイン + タブの設定ダイアログの項目)
struct TabState {
	PaneTabState panes[2];  //!< [0]=左ペイン, [1]=右ペイン (MAX_FILELIST に相当)
	UnicodeString caption;  //!< カスタムキャプション (TabList CSV の [2] に相当。
	                        //!< 空ならディレクトリ名から作る。F:TabDlg で編集)
	UnicodeString icon;     //!< タブのアイコン (CSV の [3]。F:TabDlg で編集。
	                        //!< 表示には使わない。未実装扱い)
	int work_mode = 0;      //!< ワークリストの指定 (CSV の [6]。0=使わない/1=現在/2=指定)
	UnicodeString work_list;  //!< 指定のワークリスト (CSV の [7]。work_mode==2 のときだけ有効。
	                          //!< 読み込み自体は未移植のため保持のみ。未実装扱い)
};
