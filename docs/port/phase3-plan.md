...
## 4. 後回しにする範囲 — **決定済み 2026-08-21**

メンテナの判断: **「後回しで良い。コア機能でないとは思っている」**。
以下は他をすべて片付けてから着手する。**やらないとは決めていない。**
...
合計は F モードで 20コマンド程度 (未実装 325 の 6%)。第2段の機能群 (§3) から
外し、13群を先に終わらせる。

なお `FTPConnect` などは**キーに割り当てられていても何も起きない**状態になる。
未実装のコマンドは `Execute()` が false を返してキーが通常処理に流れるだけで、
黙って別の動作をすることはない (`gui/main_frame.cpp` の Execute)。
