...
| `text_viewer_core::LoadForView` の `is_binary` が**小さなバイナリを取りこぼす** | NUL を含む6バイトのファイルがテキストと判定された。`get_MemoryCodePage()` の判定はある程度の長さを前提にしているとみられる。文字コード変換 (破壊的) がこれだけを見ていたため**バイナリを書き換えてしまった**。テストで発覚し、NUL バイトの検査を足して二重にした (Phase 3 第2段) |
| `file_ops::RenameItem` が**大文字小文字だけの改名を常に拒否**していた | Windows は大文字小文字を区別しないので、`a.txt` → `A.txt` のとき宛先が「既に存在する」と判定される。`NameToUpper` を足したときに発覚 (Phase 3 第2段)。単独の改名でも同じだったはず |
...
**メンテナが実機で確認し「キー入力大丈夫そう」との回答を得た。**
見つかったのも直ったことが確認できたのも実機で、CI とテストはどちらの局面でも
役に立っていない。この経路の自動検証は依然として無い。
...
## 21. 結果リストの項目に対するファイル操作が別のファイルを指していた

**Phase 3 で自分が作り込んだ不具合。破壊的。** ワークリストの設計中に
`gui/main_frame.cpp` を読み直して見つけた (規約9 のとおり、**テストが1件も無い
経路**だった)。

### 症状

`FilePane::GetSelectedNames()` は項目の**名前だけ**を返す。呼び出し側は
一律に `pane->GetPath() + name` でフルパスを組み立てていた。

通常の一覧ならこれで合っているが、**結果リスト** (ファイル名検索・grep 結果・
重複ファイル検索) の項目は**別々のディレクトリにある**。`GetPath()` は
「結果リストに入る直前に開いていたディレクトリ」を返すだけなので、

- 検索結果で `D:\work\deep\memo.txt` にカーソルを置いて `D` (削除) を押すと、
  **`C:\here\memo.txt` (直前のディレクトリの同名ファイル) がゴミ箱へ行く**
- 同名が無ければ「失敗」で済むが、**同名があると黙って別のファイルを消す**

コピー・移動・リンク作成・ハッシュ・文字コード変換 (その場で書き換える)・
一括リネームも同じ経路だった。合計 28 箇所。

### 直し方

判断を wx 非依存の純関数に切り出して固定した (規約8)。

    UnicodeString FullPathOfItem(const UnicodeString &dir, const FileItem &item);
    // full_path が入っていればそちらを使う。無ければ dir + name

`FilePane` 側に `GetSelectedPaths()` (フルパスの一覧) と `CurrentFullPath()` を
足し、`GetPath() + 名前` の 28 箇所をすべて置き換えた。`GetSelectedNames()` は
**確認ダイアログの表示専用**とヘッダに明記した。

さらに、「このペインのディレクトリ」そのものが要る操作
(ディレクトリ作成・ファイル作成・貼り付け・一括リネーム・テキスト結合) は
結果リストの上では成立しないので、`RejectOnResultList()` で理由を出して断る。
VCL も結果リスト上のこれらを `USTR_CantOperate` で断っている。

### 検証の範囲 (未検証を明記する)

- `FullPathOfItem` の 4 ケースは `tests/core/test_gui_file_item.cpp` で固定した
- **呼び出し側 28 箇所そのものにテストは無い**。`FilePane` は wx に依存するので
  `core_tests` からは触れない。置き換えの網羅は grep (`GetPath() + `) で確認した
  だけで、**実機での確認は削除経路のみ**

### 教訓

規約9 の「新しい層を足したら、この経路のテストは何件あるかを先に数える」に
結果リスト (#22 / #23 で追加) が引っかかっていなかった。**リストの出どころが
増えたのに、対象パスの決め方を1箇所にまとめていなかった**のが原因。
「名前」と「フルパス」を同じ型 (`UnicodeString`) で持ち回していたので、
取り違えてもコンパイルが通ってしまう。

---

## 22. ワークリスト (Phase 3 機能群14)

「別々のディレクトリにあるファイルを集めて持ち歩く一覧」。VCL では
`Global.cpp` の `WorkList` (グローバル1本の `TStringList`) が中身で、
左右どちらのペインにも同じものを出せる。
...
| 項目 | VCL の実装 | 合わせた内容 |
|---|---|---|
| `.nwl` の1行 | `Global.cpp:7843` | `パス <TAB> 別名`。`;` で始まる行と空行は読み飛ばす |
| 別名が空のとき | `save_WorkList` の `cat_sprintf("\t%s", ...)` | **タブは必ず付く** (空でも省略しない) |
| ディレクトリ | `IncludeTrailingPathDelimiter` | 末尾に区切り文字を付けて書き、読むときに落とす |
| 区切り行 | `is_separator(s)` = `s=="-"` | パスが空で別名が `-` の行。**保存でも残す** |
| 文字コード | `saveto_TextUTF8` = `TEncoding::UTF8` | UTF-8 + **BOM**。落とすと VCL が ANSI と誤読する |
| 重複判定 | `WorkList->IndexOf` (`CaseSensitive` 既定 false) | 大文字小文字を区別しない |
| 1つ上/下へ | `ItemTmpUp/Down` (MainFrm.cpp:19977/20009) | 先頭 (末尾) が対象なら何もしない。選択が無ければカーソル位置の1件 |
| カーソル位置へ | `ItemTmpMove` (MainFrm.cpp:20041) | 抜き取った分だけ挿入位置を手前にずらす。動かした項目の選択は解除 |
| 並べ替え | `Global.cpp:4551` | 区切り行があるか `NotSortWorkList` なら並べ替えない → **常に並び順を保つ**ことにした |
| 絞り込み中の変更 | `USTR_WorkFiltered` (MainFrm.cpp:11004 ほか) | 断る。添字がずれた状態で動かすと見えていない項目が動く |

### 破壊的な経路で気をつけたところ

- **ワークリストの上での `Delete` はファイルを消さず、一覧から外すだけ**
  (VCL も同じ。MainFrm.cpp:28916)。ここを繋ぎ忘れると、登録した実ファイルが
  ゴミ箱へ行く
- 区切り行と実体の無い項目は `GetSelectedNames()` / `GetSelectedItems()` から
  外した。**名前が空の項目がコピー・削除の対象に混ざらないようにする**ため
- 実体の無い項目は読み込み時に**黙って捨てない**。外付けドライブが繋がって
  いないだけかもしれないので、`<欠落>` と薄字で出して件数を知らせ、
  `WorkList_DI` で明示的に外してもらう

### コマンドの引数 (`_` の後ろ) に対応した

VCL は `ActionParam` を `コマンド名_引数` の形で持つ (`src/usr_cmdlist.h` の
`get_CmdStr` / `get_PrmStr`)。`MainFrame::Execute` でこれを分けるようにした。
**表の 464 個にコマンド名へ `_` を含むものは1つも無い**ので区切りは一意。
`scripts/check_commands.py` も同じ規則で切ってから照合する。

対応した引数: `WorkList_OP` (反対側に出す) / `WorkList_DI` (欠落を外す) /
`SaveAsWorkList_FL` (現在の一覧を書き出す)。

### 検証の範囲 (未検証を明記する)

- `gui/work_list.cpp` は 37 ケース / 105 アサーションで固定した
  (`tests/core/test_gui_work_list.cpp`)。書式の往復・並べ替え・BOM・
  日本語の別名・欠落の扱いを含む
- **`gui/main_frame.cpp` 側の受け渡しにテストは無い** (wx に依存するため)。
  規約9 に従って明記する。特に次は読んで確かめただけ:
  - 絞り込み中の禁止 (`RejectWhenWorkFiltered`)
  - ペイン側が勝手に一覧へ戻ったときの追従 (`WorkPane()` の自己修復)
  - 終了時の「保存しますか?」で「中止」を選んだときの `Veto`
- キー割り当ては `F:W=WorkList` (Global.cpp:2096) 以外**すべて推測**

---

## 23. 栞マークとタグ (Phase 3 機能群15)

**保存の仕組みは1行も書き直していない。** どちらも移植済みのものをそのまま使う。

| | 実体 | 保存先 |
|---|---|---|
| 栞マーク | `UsrIniFile::FileMark` / `IsMarked` / `MarkIdxList` (src/UIniFile.cpp) | ini のセクション (ディレクトリ名) に `名前 <TAB> メモ <TAB> 日時` |
| タグ | `TagManager` (src/usr_tag.cpp) | 実行ファイルと同じ場所の `TAGDATA.TXT` |

`gui/bookmarks.h` に置いたのは「次/前の栞へどう折り返すか」だけで、そこは
wx にも ini にも依存しない純関数にしてテストで固定した (規約8)。

### 実測して合わせたところ

`NextMark` (MainFrm.cpp:22380) の折り返しに癖がある。後ろに栞が無ければ
**「カーソル位置以前で最初に見つかった栞」**へ飛ぶ。判定が `i<=c_idx` で
**カーソル位置そのものを含む**ので、最後の栞の上にいて後ろに栞が無いときは
その場に留まる。読まずに書いたら「先頭の栞へ飛ぶ」と実装していたはず。

### VCL と変えたところ (理由付き)

| | VCL | ここ | 理由 |
|---|---|---|---|
| 栞の ini | `NyanFi.ini` | `<exe名>_wx.ini` | GUI 版の既定の方針 (gui/settings.h)。**VCL 版で付けた栞は見えない** |
| 栞の見せ方 | 行全体を `col_bgMark` で塗る | 行の左端に細い帯 | 選択色・インクリメンタルサーチの一致色と塗りが重なるため |
| `MarkMask` | `SelMaskList` で一覧そのものを絞る | 名前を並べたパスマスクを組み立てて掛ける | こちらには絞り込みの仕組みがパスマスクしか無い |
| `FindMark` | 検索の仕組み (`find_MARK`) に載せる | ini の栞のうちカレント配下のものを結果リストに出す | 検索基盤が未移植。**再帰的に走査しないので速い**が、ini に無い栞は出ない |
| `TagManager` の生成 | 起動時 (Global.cpp:1442) | 初めて使うときだけ | タグを使わない人の起動を遅くしないため。挙動は変わらない |

### 検証の範囲 (未検証を明記する)

- `gui/bookmarks.cpp` は 14 ケース / 45 アサーションで固定した
  (`tests/core/test_gui_bookmarks.cpp`)。折り返しの癖・メモ・ini への往復・
  実体が消えた栞の整理を含む
- **タグ側にこの層のテストは無い。** `TagManager` 自体は
  `tests/core/test_usr_tag.cpp` が押さえているが、`gui/main_frame.cpp` の
  受け渡し (共通タグの既定値の求め方、AND 検索での選択) は**読んだだけ**
...
`ClearLog` / `ListLog` / `ShowLogWin` / `ViewLog` / `LogFileInfo` / `ListNyanFi`。

`gui/log_win.h` は wx に依存せず、**ログ行の状態・書式・上限**だけを持つ。

### 実測して分かったこと: 行の先頭1文字が状態

VCL のログは1行の**先頭1文字**が状態を表し、処理の途中で
`msg[1] = 'E';` のように**その1文字を書き換えて**結果を記録している
(`src/Global.cpp` の `make_LogHdr` / `set_LogErrMsg`、`src/MainFrm.cpp` の各所)。
実測した文字は次の7種:
...
`MainFrame::LogResult()` を足してコピー・移動・削除の結果を記録する。
**削除は消したものの一覧も残す** (消えたファイルは後から確認しようがないため)。

### まだ無いもの (無言でスキップしない)

- **ログ専用ウィンドウが無い。** VCL は独立したログ窓を持ち、
  `ScrollUpLog` / `ScrollDownLog` / `ToLog` はその窓を操作するコマンド。
  窓が無いので**この3つは実装していない**
- `ShowLogWin` は本来「窓の表示/非表示」だが、**一覧表示 (`ListLog`) と
  同じ動きにしてある**。押しても何も起きないより、中身が見える方がましと判断した
- `ListLog` は `wxMessageBox` に流し込むだけなので **300行で打ち切る**
  (打ち切ったことは画面に出す)
- `ViewLog` は一時ファイルに書いてテキストビューアで開く。
  **その一時ファイルは消さない** (ビューアが読んでいる間は消せない)

### 検証の範囲

- `gui/log_win.cpp` は 38 ケース (core 全体: 821 ケース / 2,557 アサーション)。
...
## 30. 名前を付けた状態の保存と読み込み (Phase 3 機能群21)

`SaveTabGroup` / `SaveAsTabGroup` / `LoadTabGroup` /
`SaveAsResultList` / `LoadResultList`。

### 実測して分かった書式

| 種類 | 拡張子 | 文字コード | 書式 |
|---|---|---|---|
| 結果リスト | `.txt` | **UTF-8 + BOM** (`saveto_TextUTF8`) | `;[ResultList]` の見出し + `;Find_Path=...` + `パス <TAB> 別名` (`.nwl` と同じ行書式) |
| 検索設定 | `.ini` | `UsrIniFile` の既定 | `FindSettings` セクション。キー名は `save_FindSettings` / `load_FindSettings` (Global.cpp:5192/5360) と同じ |

- **VCL は結果リストを読むとき、実体の無い項目を黙って読み飛ばす**
  (MainFrm.cpp:21485)。ワークリストの「欠落として残す」方針とは逆だが、
  既存の挙動なのでそのまま踏襲した (規約6)
- アイコン一覧 (`Icons`) は ini に書くとき `\r\n` と `/` を相互変換している。
  担当が最初のドラフトで見落とし、自己レビューで見つけて直した

### タブグループの書式は新規設計 (実測に合わせられなかった)

VCL の `TabList` は CSV だが、`gui/tabs.h` の `TabState` は VCL の `tab_info` と
**持っている項目が違う** (選択状態・ホームパス・階層同期を持たない)。
既存ファイルを読めるようにするのは無理と判断し、
`TabManager::SaveToIni` と同じキー命名で別セクション (`TabGroup`) に書く
**新しい書式**にした。**VCL 版のタブグループファイルは読めない。**

### 入れなかったもの (無言でスキップしない)

- **`LoadFindSet` / `SaveAsFindSet` はコマンドとして繋いでいない。**
  読み書きの土台 (`named_state::FindSet`、VCL のキー名を実測して 30 項目) は
  用意してあるが、**Phase 2 に検索の設定を持つ仕組みがまだ無く、
  読み込んでも当てはめる先が無い**
- 検索設定の拡張項目 (EXIF/GPS/タグ/画像サイズ/ハッシュなど約25項目) は
  同じ理由で構造体にも入れていない
- 結果リストの検索条件メタ情報 (`Find_Mask` / `Find_Keywd` など) も同上。
  保持しているのは `Find_Path` だけ

### 検証の範囲

- `gui/named_state.cpp` は 34 ケース (core 全体: 875 ケース / 2,763 アサーション)。
  3種類それぞれの往復と BOM を含む
- UNC パスの可用性チェック (`is_InvalidUnc`) は入れていない。
  `Global.cpp` を巻き込んでリンク問題を起こすため (§24 と同じ穴)。
  単純な存在確認だけにしてある
- `gui/main_frame.cpp` 側の受け渡しにテストは無い (wx 依存)
- キー割り当ては全部推測
